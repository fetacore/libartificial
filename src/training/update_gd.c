...
void update_gd(int rows, int columns_Y, int columns_X, int batch, int layers,
							 int nodes[layers],
	...
	switch (batch == rows) {
		...
		case 1:
			...
			for (e = epochs; e--; ) {
				...
				feedforward_update(Z, rows, columns_Y, columns_X, layers,
													 X, wb, nodes, funcs);
					...
					for (r = 0; r < batch; r++) {
						...
						switch (layers == 1) {
							...
							default:
								for (l = 1; l < layers; l++) {
									for (j = 0; j < nodes[l]; j++) {
										Z_batch[0][l][r * nodes[l] + j] =
																								Z[0][l][(r + b) * nodes[l] + j];
										Z_batch[1][l][r * nodes[l] + j] =
																								Z[1][l][(r + b) * nodes[l] + j];
									}
								}
								break;
						...
						for (j = 0; j < columns_Y; j++) {
							Z_batch[0][layers][r * columns_Y + j] = 
																					Z[0][layers][(r + b) * columns_Y + j];
							Z_batch[1][layers][r * columns_Y + j] =
																					Z[1][layers][(r + b) * columns_Y + j];
						}
					}
					
					// Fill the deltas
					delta_gd(deltas, batch, columns_Y, layers,
									 Y_batch[i], Z_batch, wb, nodes, funcs);
