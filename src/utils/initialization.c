...
double **init_w(const double variance, const int layers,
                ...
                const int columns_Y, const int columns_X)
{
  ...
  double **weights = malloc((layers + 1) * sizeof(double *));
    ...
    switch (l > 0 && l < layers) {
      ...
      case 0:
        switch (l == 0) {
          ...
          case 1:
            multiplication = columns_X * nodes[l];
            weights[l] = malloc(multiplication * sizeof(double));
